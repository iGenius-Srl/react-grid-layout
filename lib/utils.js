...
export function dynamicCompact(layout: Layout, cols: number): Layout {
  ...
  sorted = _horizontalCompact(sorted, cols);
  console.log('sorted after hori', sorted)
  let test = sortLayoutItemsByRowCol(sorted);
  for (let i = 0; i < 121; i++) {
    test = _horizontalCompact(test, cols);
    test = sortLayoutItemsByRowCol(test);
  }
  console.log('sorted 2 ', test)
  return test;
}
...
function _horizontalCompact(layout: Layout, cols: number): Layout {
  ...
  for (let i=1; i < layout.length; i++) {
    ...
    if (item.y === prevItem.y && prevItem.x + prevItem.w < cols) {
      newLayout.push({
        ...item,
        x: prevItem.x + prevItem.w,
      });
    } else if ( prevItem.x + prevItem.w + item.w <= cols ) {
      newLayout.push({
        ...item,
        x: prevItem.x + prevItem.w,
        y: prevItem.y
