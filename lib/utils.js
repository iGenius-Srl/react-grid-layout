...
export function cloneLayoutItem(layoutItem: LayoutItem): LayoutItem {
  return {
    ...
    isSection: layoutItem.isSection
  };
}
...
export function dynamicCompact(layout: Layout, cols: number): Layout {
  // We go through the items by row and column.
  let test = sortLayoutItemsByRowCol(layout);
  console.log(test);
...
function _horizontalCompact(layout: Layout, cols: number): Layout {
  ...
  for (let i=0; i < layout.length; i++) {
    let item = layout[i];
    item = _placeItem(item, newLayout, cols, sectionY);
    if(item.isSection) {
      sectionY = item.y;
    }
    newLayout.push({
    ...
    //newLayout = compact(newLayout, true);
  }

  return newLayout;
}

function _placeItem(item: LayoutItem, layout: Layout, cols: number, sectionY: number): LayoutItem {
  // items on the same lines
  // starts from section y position + 1.
  for (let y = sectionY; true; y++) {
    for (let x = 0; x <= (cols - item.w); x++) {
      item.x = x;
      item.y = y;
      if (!getFirstCollision(layout, item)) {
        return item;
      }
    }
    if (y > 120) {
