...
function _horizontalCompact(layout: Layout, cols: number): Layout {
  ...
  for (let i=1; i < layout.length; i++) {
    let item = layout[i];
    item = _placeItem(item: LayoutItem, newLayout: Layout, cols: number);
    newLayout.push({
      ...item
    });
...
function _placeItem(item: LayoutItem, layout: Layout, cols: number): LayoutItem {
  // items on the same lines
  for (let y = 0; true; y++) {
    for (let x = 0; x <= (cols - item.w); x++) {
      item.x = x;
      item.y = y;
      if (!getAllCollisions(layout, item).length) {
        return item;
      }
    }
    if (y > 120) {
